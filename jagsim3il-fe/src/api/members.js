...
// 예상 API: POST /api/challenges/:challengeId/join
// request:  { goal }
// response: ChallengeMember
export async function submitGoal(challengeId, goal) {
  const member = {
    id: `rm${Date.now()}`,
...
// 인증 이미지/동영상 업로드
// 예상 API: POST /api/challenges/:challengeId/members/:memberId/media  (multipart/form-data)
...
export async function uploadMedia(challengeId, memberId, asset) {
  const media = {
    uri: asset.uri,
