jagsim3il이라는 React + Vite + Tailwind CSS 프로젝트의 프론트엔드를 만들어줘.
...
- Header
  - 서비스명 로고/텍스트
  - Room 추가 버튼 (클릭 시 room 추가 모달 오픈)
  - 설정 페이지 이동 버튼
- Body
  - 내가 참여 중인 방 리스트 (카드 그리드)
    - 각 카드: 방 제목, 타이틀(설명), 참여 중인 멤버 아바타/목록
    - 카드 클릭 시 해당 방 페이지(/room/:roomId)로 이동

## 4. Room 추가 모달 (홈 화면에서 오픈되는 모달 컴포넌트)
- 시작 날짜 및 시간 설정 (date + time picker)
- 종료 날짜 및 시간 설정 (date + time picker)
- 패널티 설정 (텍스트 입력 또는 프리셋 선택)
- 생성 버튼 클릭 시 mock으로 방 리스트에 추가하고 모달 닫기

## 5. 방 페이지 (/room/:roomId)
이 페이지는 "처음 입장"과 "이후 입장" 두 가지 상태를 분기해서 보여줘야 해.
mock 상태값(예: hasJoined boolean)으로 분기 처리.

### 5-1. 처음 입장 시 (목표 설정 화면)
- 종료 기한까지 수행할 목표를 입력하는 폼
- 입력 후 "입장하기" 버튼 클릭 시 이후 입장 화면으로 전환

### 5-2. 이후 입장 시 (메인 방 화면)
- Header
  - 방 이름
  - 상세 버튼 (클릭 시 방 정보 상세 모달 또는 페이지로 이동 - 간단한 모달로 처리)
- Body
  - 멤버 카드 리스트 (그리드)
    - 각 멤버 카드 구성:
      ...
      - 하단: 이미지/동영상 업로드 또는 촬영 영역
        - 파일 업로드 input (image/video accept)
        - 카메라 촬영 버튼 (getUserMedia 활용한 간단한 캡처 UI, 동작 mock이어도 됨)
        - 업로드된 파일 미리보기 표시
- Footer
  - 이 방에 설정된 패널티를 보여주는 오버레이/배너 (화면 하단 고정, 항상 노출)

# 공통 요구사항
- 반응형 디자인 (모바일 우선, 카드 리스트는 모바일에서 1열, 데스크탑에서 2~3열)
- 라우팅 구조를 React Router로 명확히 잡아줘 (/login, /signup, /, /room/:roomId, /settings)
- 컴포넌트는 기능 단위로 분리 (예: components/RoomCard, components/MemberCard, components/CreateRoomModal 등)
- mock 데이터는 src/mocks 디렉토리에 분리
- 디자인은 깔끔하고 현대적인 느낌으로, Tailwind의 둥근 모서리/그림자/여백을 적절히 활용해줘
- 모든 텍스트는 한국어로 작성

# 작업 순서 제안
1. 프로젝트 초기 세팅 (Vite + React + Tailwind + React Router + zustand)
2. 라우팅 구조 및 폴더 구조 설계
3. mock 데이터 설계 (user, room, member)
4. 로그인/회원가입 페이지 구현
5. 홈 화면 + Room 추가 모달 구현
6. 방 페이지 (처음 입장 / 이후 입장 분기) 구현
7. 반응형 스타일 점검
8. 코드 분석 문서 작성 (.md)

각 단계마다 만든 컴포넌트/페이지를 간단히 설명해주고, 전체 작업이 끝나면 실행 방법(npm run dev)도 알려줘.
...
- Room 추가(생성) 요청
- 방 페이지 진입 시 방 정보 및 멤버 리스트 조회
- 처음 입장 시 목표 입력 후 제출(방 참여 등록)
- 멤버 카드에서 이미지/동영상 업로드
- 방 상세 정보 조회 (상세 버튼 클릭 시)
- 설정 페이지 관련 조회/변경 동작
- 패널티 정보 조회
